...
- `POST /api/v1/admin/targets` body `{ "agent_id", "endpoint", "collector_type", "credential_id?", "label?", "enabled" }`. `collector_type` is one of `ssh`, `snmp`, `http`, `cert`, `bacnet`, `modbus`, `google-wifi`, `technitium-dns`, `home-assistant`. `endpoint` is a bare host/IP for device-style collector types, or a full URL for service-style ones (`technitium-dns`, `home-assistant`).
- `PUT /api/v1/admin/targets/{id}` body `{ "endpoint", "collector_type", "credential_id?", "label?" }` — update a target (REQ-006). `enabled` is toggled via the dedicated endpoint below.
