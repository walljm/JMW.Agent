...
Owner decision D2 narrows, rather than reverses, that stance: when an IP address is observed for two different device identities **within the same collection cycle**, there is no time gap in which a DHCP reassignment could have occurred — the IP was, provably, in use by one physical host at one instant, observed once. Correlating on it under those conditions is as safe as a fingerprint match and is approved as a general corroboration signal (any fingerprint type), not one limited to SSH.

**Implemented** (`FactsEndpoint.HandleAsync` + `DeviceRegistry.MergeCorrelatedByIpWithConnectionAsync`), scoped deliberately narrower than "the whole collection cycle": only to interface-IP facts (`FactPaths.InterfaceIPv4`, `FactPaths.SshInterfaceIP`) submitted directly in the batch elements of the SAME `POST /api/v1/agent/facts` request. Devices resolved synchronously in that request's per-batch-element loop that share an IP asserted in their own facts are merged (oldest survives, audit action `device.auto_merge_ip_same_cycle`), with alias resolution per merge group so an id merged away by one IP group mid-request resolves correctly if a second group also references it.

Deliberately **not** extended to the trailing `DiscoveryMaterializer` pass (ARP/DHCP/discovered-row resolution) or to any read of `proj_interfaces`/`proj_systems` projection state, even though both run within the same request: a device counted as "touched" this cycle can still have a stale IP surviving on its row from days ago, because `UpsertDeviceSystemAsync`/routed projection upserts use `COALESCE` — an update that doesn't supply a fresh IP silently preserves whatever was there before. Reading current projection state can't distinguish "this IP was just reasserted" from "this row was touched for an unrelated reason and an old IP is still sitting in the column," which would reopen exactly the DHCP-reassignment risk D2 is meant to exclude. Closing that gap for the materializer's passive sources is tracked as follow-up work, and needs either an explicit freshness check (row `updated_at` against the request's `CollectedAt`) or restructuring those passes to carry the IP forward from the row they just read rather than re-reading projection state after the fact.

The immediate motivating bug (a credentialed SSH probe never converging with a passive scan of the same host) turned out to be a separate defect — the credentialed collector was emitting a fabricated fingerprint instead of the real host-key hash — fixed independently in `SshCollector.cs`.
