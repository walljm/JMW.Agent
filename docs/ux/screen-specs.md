...
- **2. FLEET HEALTH (two columns).**
  ...
  - **Collection Health** — fleet rollup of the most recent *real* `agent_cycles` per agent (heartbeat-only ticks with no collector/scanner/service activity are excluded, or the "last cycle" stat trivially reads 0 the moment every agent's newest tick happens to be a no-op heartbeat): total facts sent, count of agents reporting `error_count > 0`, average cycle duration, and an inline-SVG **error-rate sparkline** over the last N cycles (7-day retention window). Below that, a fleet-wide **two-series daily trend** — facts sent per day (raw volume, summed across all agents) plotted against confirmed changes per day (`facts_history` rows, i.e. values that actually differed from the prior known one) — with a per-series N-day average. A persistent gap between the two lines signals resend noise (e.g. an agent's on-disk delta cache was reset) rather than genuine network drift; this replaces the standalone Network-section "Change trend" panel below, whose bare diff-count sparkline duplicated a job Recent Activity already does with real narration. Header action → agent detail / cycles.
...
**Data shown:** posture rollup counts (certs, updates, hardware/SMART, containers/services/filesystems, security gaps, fingerprint conflicts); agent health (heartbeat-derived online/offline, status, version, zone, passive mode); collection rollup (facts_sent, agents-with-errors, avg duration, error-rate series, daily facts-sent/confirmed-changes trend); recent changes; new devices; not-seen devices; device/agent totals; composition group-bys; last-updated timestamp; staleness flags.
