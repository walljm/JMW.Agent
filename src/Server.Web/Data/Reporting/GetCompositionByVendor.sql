...
-- et al. — see docs/plans/vendor-derivation-updates.md §3) for devices no protocol self-reports a
-- vendor for. NULL vendors group together and are labelled by the caller. Excludes merged/alias
-- devices.
SELECT
    coalesce(pdv.vendor, hw.system_vendor, pdv.vendor_guess) AS vendor
  , count(*) AS COUNT
FROM
    live_devices d
...
GROUP BY
    COALESCE (pdv.vendor, hw.system_vendor, pdv.vendor_guess)
ORDER BY
    COUNT  DESC
  , vendor ASC
