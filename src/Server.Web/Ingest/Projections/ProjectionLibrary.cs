...
/// <summary>
/// Declares all projection tables for the JMW Agent network discovery system.
/// Each projection maintains a queryable current-state table derived from incoming facts.
/// <see cref="AllDefs" /> is the single source of truth for projection shape — DeviceRegistry's
/// merge logic reads it directly to discover which tables carry device-scoped state, so adding
/// a projection here automatically makes it merge-aware with no second list to update.
/// </summary>
public static class ProjectionLibrary
{
    public static readonly IReadOnlyList<ProjectionDef> AllDefs =
    [
        new ProjectionDef(
            "proj_devices",
            ["Device"],
