...
public sealed class GenericProjection : IProjection
{
    ...
    // ── Entity state cache ────────────────────────────────────────────────────
    //
    // EntityStateCache moved to its own file (EntityStateCache.cs) so
    // ContextDerivationEngine can share the identical change-suppression mechanism
    // (docs/plans/context-derivations.md §3.1).
